...
{{-- vie goes here --}}
    ...
    <div class="container-fluid px-4">
        ...
        @include('screens.management.OrganisationAdmin.views.group_member_view')
</div>
</div>
...
$(document).ready(function() {
    ...
    $('.delete-badge').click(function(e) {
        ...
        if (confirm('Are you sure you want to delete this event?')) {
            ...
            $.ajax({
                url: '/organisation/delete-event/' + groupid,
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
    ...
    // suspend group member
    $('.suspend-badge').click(function(e) {
        e.preventDefault();
        let groupid = $(this).data('id');
        if (confirm('Are you sure you want to suspend this member?')) {

            $.ajax({
                url: '/organisation/suspend-member/' + groupid,
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
    ...
    $('.activate-badge').click(function(e) {
        e.preventDefault();
        let groupid = $(this).data('id');
        if (confirm('Are you sure you want to activate this member?')) {

            $.ajax({
                url: '/organisation/activate-member/' + groupid,
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
